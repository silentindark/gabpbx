...
exten => s,2,MYSQL(Connect connid localhost gabpbx mypass credit utf8)
...
static int set_gabpbx_int(struct ast_channel *chan, char *varname, int id)
{
	if (id >= 0) {
		char s[12] = "";
...
static int add_identifier_and_set_gabpbx_int(struct ast_channel *chan, char *varname, int identifier_type, void *data)
{
	return set_gabpbx_int(chan, varname, add_identifier(chan, identifier_type, data));
}
...
static int aMYSQL_connect(struct ast_channel *chan, char *data)
{
	...
	add_identifier_and_set_gabpbx_int(chan, args.connid, AST_MYSQL_ID_CONNID, mysql);
	return 0;
}
...
static int aMYSQL_query(struct ast_channel *chan, char *data)
{
	...
	if ((mysqlres = mysql_store_result(mysql))) {
		add_identifier_and_set_gabpbx_int(chan, args.resultid, AST_MYSQL_ID_RESID, mysqlres);
		return 0;
	...
	if (args.resultvar && (resultid >= 0) ) {
		if ((mysqlres = find_identifier(resultid, AST_MYSQL_ID_RESID)) != NULL) {
			...
			if ((mysqlrow = mysql_fetch_row(mysqlres)) != NULL) {
				...
				set_gabpbx_int(chan, args.resultvar, 1); /* try more rows */
			} else {
				ast_debug(5, "ast_MYSQL_fetch : EOF\n");
				set_gabpbx_int(chan, args.resultvar, 0); /* no more rows */
			}
			return 0;
		} else {
			set_gabpbx_int(chan, args.resultvar, 0);
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Simple Mysql Interface");
