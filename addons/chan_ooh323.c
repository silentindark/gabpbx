...
#define DEFAULT_LOGFILE "/var/log/gabpbx/h323_log"
...
static struct ooh323_pvt *ooh323_alloc(int callref, char *callToken) 
{
	...
	if (!(pvt->rtp = ast_rtp_instance_new("gabpbx", sched, &tmp, NULL))) {
	...
	(ooCallData *call, struct ast_codec_pref *prefs)
{
	...
	while (codec) {
		ooAppendCapToCapPrefs(call, ooh323_convertGABpbxCapToH323Cap(codec));
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Objective Systems H323 Channel");
