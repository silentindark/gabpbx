	...
	if (var) {
		...
		if (!ast_strlen_zero(app)) {
			...
			if (a) {
				...
				if (chan && chan->cdr)
                                        ast_verb(3, "[%s|%s] Executing [%s@%s:%d] %s(\"%s\", \"%s\")\n",
                                                         chan->cdr->clisrc, chan->cdr->clidst, chan->exten, chan->context, chan->priority,
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Realtime Switch");
