...
GABPBX_FILE_VERSION(__FILE__, "$Revision: 365460 $")
...
static int handle_recordfile(struct ast_channel *chan, AGI *agi, int argc, const char * const argv[])
{
	...
	const char *silencestr = NULL;	/* T42.3: receives strchr(argv[N],...) const result; only read (strlen + indexing + atoi); ++ rebinds the pointer, doesn t write through it */
	int rfmt = 0;
