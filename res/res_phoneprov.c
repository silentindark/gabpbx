...
static void build_profile(const char *name, struct ast_variable *v)
{
	...
	for (; v; v = v->next) {
		if (!strcasecmp(v->name, "mime_type")) {
			...
			const char *file_extension;	/* T42.3: receives strrchr(pp_file->format,...) const result; only read by ast_http_ftype2mtype */
			char *value_copy = ast_strdupa(v->value);
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "HTTP Phone Provisioning",
		.load = load_module,
