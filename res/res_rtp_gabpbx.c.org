...
static int bridge_p2p_rtp_write(struct ast_rtp_instance *instance, unsigned int *rtpheader, int len, int hdrlen)
{
	...
	// germanico, aquí hay que leer secuencia
	...
	ast_verbose("Germanico p2p RTP (seq %-6.6u, lastseq %-6.6u)\n", seqno, prev_seqno);
//	ast_verbose("Germanico p2p RTP (seq %-6.6u\n", seqno);
...
static struct ast_frame *ast_rtp_read(struct ast_rtp_instance *instance, int rtcp)
{
	...
	// germanico lost any incoming rtp paket?
	ast_verbose("Germanico RTP (seq %-6.6u, lastseq %-6.6u)\n", seqno, prev_seqno);
