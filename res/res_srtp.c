...
/* See https://wiki.gabpbx.org/wiki/display/AST/Secure+Calling */
...
static struct ast_srtp_policy *ast_srtp_policy_alloc()
{
	...
	/* T39: libsrtp 2.x's srtp_add_stream rejects window_size=0 with
	 * srtp_err_status_bad_param even though srtp.h:342 documents 0 as "use
	 * default 128". Set explicit non-zero replay window to satisfy the stricter
	 * validation. 64 is conservative — smaller than libsrtp's default 128
	 * (less memory per stream, faster validation under fork-N policies)
	 * and well below SRTP_MAX_REPLAY_WINDOW (1024). */
	tmp->sp.window_size = 64;
...
{
	/* T38: libsrtp 2.x removed the per-field cipher_type/auth_type setters
	 * (no AES_128_ICM / HMAC_SHA1 enum values). Use the canonical helpers
	 * which set cipher + auth + tag length + sec_serv atomically. */
	switch (suite) {
...
static int ast_srtp_get_random(unsigned char *key, size_t len)
{
	/* T38: libsrtp 2.x removed crypto_get_random — pull from OpenSSL instead.
