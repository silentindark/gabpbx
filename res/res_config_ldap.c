	...
	const char *name, const char *value)
{
	char *new_name = NULL;
	char *new_value = NULL;
	const char *like_pos = strstr(name, " LIKE");	/* T42.3: only read for offset (like_pos - name); name itself is strdupa-d below */
      ...
      const char *table_name, va_list ap)
{
	...
	/* T42.3: initfield is writable (just strdupa-d); cast accepted because
	 * the const is propagated from initfield's declared type, but the
	 * underlying buffer is heap-allocated by ast_strdupa above. */
...
static int parse_config(void)
{
	...
	if (!ast_strlen_zero(user)) {
		if (!(s = ast_variable_retrieve(config, "_general", "pass"))) {
			ast_log(LOG_WARNING, "No directory password found, using 'gabpbx' as default.\n");
