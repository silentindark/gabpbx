 ...
 *
 * res_config_sqlite is a module for the GABpbx Open Source PBX to
 * support SQLite 3 databases. It can be used to fetch configuration
 * from a database (static configuration files and/or using the GABpbx
 * RealTime Architecture - ARA).  It can also be used to log CDR entries. 
 * Note that GABpbx already comes with a module named cdr_sqlite.
 ...
 * simple module for GABpbx, the other is to have the same database open for
 * all kinds of operations, which improves reliability and performance.
 *
 * \section conf_sec Configuration
 *
 * The main configuration file is res_config_sqlite3.conf. It must be readable or
 ...
 * GABpbx documentation. The file tables.sql can be used to create the
 * needed tables.
 *
 * \section status_sec Driver status
 *
 * The CLI command <code>show sqlite3 status</code> returns status information
...
#include "gabpbx.h"
GABPBX_FILE_VERSION(__FILE__, "$Revision: 361471 $")
...
#define RES_CONFIG_SQLITE_NAME "res_config_sqlite3"
#define RES_CONFIG_SQLITE_DRIVER "sqlite3"
#define RES_CONFIG_SQLITE_DESCRIPTION "Resource Module for SQLite 3"
#define RES_CONFIG_SQLITE_CONF_FILE "res_config_sqlite3.conf"
 ...
 *	 error = sqlite3_exec(db, query, NULL, NULL, &errormsg);
...
/*!
 * \brief GABpbx callback function for CDR support.
 * \param cdr the CDR entry GABpbx sends us.
 *
 * GABpbx will call this function each time a CDR entry must be logged if
...
/*!
 * \brief GABpbx callback function for static configuration.
 *
 * GABpbx will call this function when it loads its static configuration,
...
/*!
 * \brief GABpbx callback function for RealTime configuration.
 *
 * GABpbx will call this function each time it requires a variable
...
/*!
 * \brief GABpbx callback function for RealTime configuration (variable
 * update).
 *
 * GABpbx will call this function each time a variable has been modified
 ...
 * create/store).
 *
 * GABpbx will call this function each time a variable has been created
...
/*!
 * \brief GABpbx callback function for RealTime configuration (destroys
 * variable).
 *
 * GABpbx will call this function each time a variable has been destroyed
...
/*!
 * \brief GABpbx callback function for the CLI status command.
 *
 * \param e CLI command
 * \param cmd 
...
/*!
 * The structure specifying all callback functions used by GABpbx for static
...
static struct ast_cli_entry cli_status[] = {
	AST_CLI_DEFINE(handle_cli_show_sqlite_status, "Show status information about the SQLite 3 driver"),
	AST_CLI_DEFINE(handle_cli_sqlite_show_tables, "Cached table information about the SQLite 3 driver"),
};
...
/*
 * Taken from GABpbx 1.2 cdr_sqlite.so.
 */
...
static struct sqlite_cache_tables *find_table(const char *tablename)
{
	...
	if ((err = sqlite3_exec(db, sql, find_table_cb, tblptr, &errstr))) {
...
static int cdr_handler(struct ast_cdr *cdr)
{
	...
	RES_CONFIG_SQLITE_BEGIN
		error = sqlite3_exec(db, ast_str_buffer(sql1), NULL, NULL, &errormsg);
	RES_CONFIG_SQLITE_END(error)
	...
	if (error) {
		ast_log(LOG_ERROR, "%s\n", S_OR(errormsg, sqlite3_errmsg(db)));
	...
	struct ast_config *cfg, struct ast_flags flags, const char *suggested_incl, const char *who_asked)
{
	...
	query = sqlite3_mprintf(sql_get_config_table, table, file);
	...
	RES_CONFIG_SQLITE_BEGIN
		error = sqlite3_exec(db, query, add_cfg_entry, &args, &errormsg);
	RES_CONFIG_SQLITE_END(error)
...
/* \endcond */
	...
	query = sqlite3_mprintf(QUERY, table, (config_table && !strcmp(config_table, table)) ? " commented = 0 AND" : "", params[0], op, vals[0]);
	...
	if (params_count > 1) {
		...
		for (i = 1; i < params_count; i++) {
			...
			tmp_str = sqlite3_mprintf("%s AND %q%s '%q'", query, params[i], op, vals[i]);
			sqlite3_free(query);
	...
	tmp_str = sqlite3_mprintf("%s LIMIT 1;", query);
	sqlite3_free(query);
	...
	RES_CONFIG_SQLITE_BEGIN
		error = sqlite3_exec(db, query, add_rt_cfg_entry, &args, &errormsg);
	RES_CONFIG_SQLITE_END(error)
	...
	const char *table, va_list ap)
{
	...
	/*
	 * GABpbx sends us an already escaped string when searching for
...
/* \endcond */
	...
	if (!(query = sqlite3_mprintf(QUERY, table, (config_table && !strcmp(config_table, table)) ? " commented = 0 AND" : "", params[0], op, tmp_str))) {
		ast_log(LOG_WARNING, "Unable to allocate SQL query\n");
		ast_config_destroy(cfg);
		ast_free(params);
		ast_free(vals);
	...
	if (!(tmp_str = sqlite3_mprintf("%s ORDER BY %q;", query, initfield))) {
		ast_log(LOG_WARNING, "Unable to reallocate SQL query\n");
		sqlite3_free(query);
		ast_config_destroy(cfg);
		ast_free(initfield);
	...
	RES_CONFIG_SQLITE_BEGIN
		error = sqlite3_exec(db, query, add_rt_multi_cfg_entry, &args, &errormsg);
	RES_CONFIG_SQLITE_END(error)
...
/* \endcond */
	...
	if (!(query = sqlite3_mprintf(QUERY, table, params[0], vals[0]))) {
		ast_log(LOG_WARNING, "Unable to allocate SQL query\n");
		ast_free(params);
		ast_free(vals);
		return -1;
	...
	if (params_count > 1) {
		...
		for (i = 1; i < params_count; i++) {
			tmp_str = sqlite3_mprintf("%s, %q = '%q'", query, params[i], vals[i]);
			sqlite3_free(query);
	...
	if (!(tmp_str = sqlite3_mprintf("%s WHERE %q = '%q';", query, keyfield, entity))) {
		ast_log(LOG_WARNING, "Unable to reallocate SQL query\n");
		sqlite3_free(query);
		return -1;
	}
	...
	va_list ap)
{
	...
	while ((param = va_arg(ap, const char *))) {
		...
		ast_str_append(&where, 0, "%s %s = %s",
			first ? "" : " AND",
			tmp1 = sqlite3_mprintf("%q", param),
			tmp2 = sqlite3_mprintf("%Q", value));
		sqlite3_free(tmp1);
		sqlite3_free(tmp2);
		first = 0;
	}
	...
	RES_CONFIG_SQLITE_BEGIN
		error = sqlite3_exec(db, ast_str_buffer(sql), NULL, NULL, &errormsg);
	RES_CONFIG_SQLITE_END(error)
...
/* \endcond */
	...
	for (i = 0; i < params_count; i++) {
		if ( tmp_keys2 ) {
			tmp_keys = sqlite3_mprintf("%s, %q", tmp_keys2, params[i]);
			sqlite3_free(tmp_keys2);
		} else {
			tmp_keys = sqlite3_mprintf("%q", params[i]);
		}
		if (!tmp_keys) {
			ast_log(LOG_WARNING, "Unable to reallocate SQL query\n");
			sqlite3_free(tmp_vals);
			ast_free(params);
			ast_free(vals);
			return -1;
		}
		...
		if ( tmp_vals2 ) {
			tmp_vals = sqlite3_mprintf("%s, '%q'", tmp_vals2, vals[i]);
			sqlite3_free(tmp_vals2);
		} else {
			tmp_vals = sqlite3_mprintf("'%q'", vals[i]);
		}
		if (!tmp_vals) {
			ast_log(LOG_WARNING, "Unable to reallocate SQL query\n");
			sqlite3_free(tmp_keys);
			ast_free(params);
			ast_free(vals);
			return -1;
		}
	...
	if (!(tmp_str = sqlite3_mprintf(QUERY, table, tmp_keys, tmp_vals))) {
		ast_log(LOG_WARNING, "Unable to reallocate SQL query\n");
		sqlite3_free(tmp_keys);
		sqlite3_free(tmp_vals);
		return -1;
	...
	RES_CONFIG_SQLITE_BEGIN
		error = sqlite3_exec(db, tmp_str, NULL, NULL, &errormsg);
	RES_CONFIG_SQLITE_END(error)

	if (!error) {
		rows_id = sqlite3_last_insert_rowid(db);
	} else {
		rows_id = -1;
	}
...
/* \endcond */
	...
	if (!(query = sqlite3_mprintf(QUERY, table))) {
		ast_log(LOG_WARNING, "Unable to allocate SQL query\n");
		ast_free(params);
		ast_free(vals);
		return -1;
	...
	for (i = 0; i < params_count; i++) {
		tmp_str = sqlite3_mprintf("%s %q = '%q' AND", query, params[i], vals[i]);
		sqlite3_free(query);
	...
	ast_free(params);
	ast_free(vals);
	if (!(tmp_str = sqlite3_mprintf("%s %q = '%q';", query, keyfield, entity))) {
		ast_log(LOG_WARNING, "Unable to reallocate SQL query\n");
		sqlite3_free(query);
		return -1;
	}
		...
		AST_RWLIST_TRAVERSE(&tbl->columns, col, list) {
			if (strcmp(col->name, elm) == 0) {
				...
				if (col->isint && !ast_rq_is_int(type)) {
					ast_log(LOG_WARNING, "Realtime table %s: column '%s' is an integer field, but GABpbx requires that it not be!\n", tablename, col->name);
					res = -1;
				}
				break;
			}
...
static char *handle_cli_show_sqlite_status(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
		e->command = "sqlite3 show status";
		e->usage =
			"Usage: sqlite3 show status\n"
			"       Show status information about the SQLite 3 driver\n";
		return NULL;
	case CLI_GENERATE:
		return NULL;
	}
	...
	ast_cli(a->fd, "SQLite3 database path: %s\n", dbfile);
...
static char *handle_cli_sqlite_show_tables(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	switch (cmd) {
	case CLI_INIT:
		e->command = "sqlite3 show tables";
		e->usage =
			"Usage: sqlite3 show tables\n"
			"       Show table information about the SQLite 3 driver\n";
		return NULL;
	case CLI_GENERATE:
		return NULL;
	}
...
static int load_module(void)
{
	...
	if ((error = sqlite3_open(dbfile, &db)) != SQLITE_OK) {
...
/* \endcond */
		...
		query = sqlite3_mprintf(QUERY, cdr_table);
		...
		if (error) {
			...
			sqlite3_free(errormsg);
			errormsg = NULL;
			query = sqlite3_mprintf(sql_create_cdr_table, cdr_table);
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Realtime SQLite configuration",
		.load = load_module,
