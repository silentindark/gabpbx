...
/*! \file
 * \brief A resource for interfacing GABpbx directly as a client
...
/*** DOCUMENTATION
	<application name="JabberSend" language="en_US">
		...
		<description>
			...
			<para>Example: JabberSend(gabpbx,bob@domain.com,Hello world) sends "Hello world"
			to <replaceable>bob@domain.com</replaceable> as an XMPP message from the account
			<replaceable>gabpbx</replaceable>, configured in jabber.conf.</para>
		</description>
		<see-also>
	...
	<function name="JABBER_RECEIVE" language="en_US">
		...
		<description>
			...
			<para>Example: ${JABBER_RECEIVE(gabpbx,bob@domain.com)} returns an XMPP message
			sent from <replaceable>bob@domain.com</replaceable> (or nothing in case of a time out), to
			the <replaceable>gabpbx</replaceable> XMPP account configured in jabber.conf.</para>
		</description>
		<see-also>
			<ref type="function">JABBER_STATUS</ref>
	...
	<function name="JABBER_STATUS" language="en_US">
		...
		<description>
			...
			<para>Example: ${JABBER_STATUS(gabpbx,bob@domain.com)} returns 1 if
			<replaceable>bob@domain.com</replaceable> is online. <replaceable>gabpbx</replaceable> is
	...
	<application name="JabberStatus" language="en_US">
		...
		<syntax>
			<parameter name="Jabber" required="true">
				<para>Client or transport GABpbx users to connect to Jabber.</para>
			</parameter>
			<parameter name="JID" required="true">
...
static int aji_register_query_handler(void *data, ikspak *pak)
{
	...
	if (!buddy) {
		...
		char *explain = "Welcome to GABpbx - the Open Source PBX.\n";
		iq = iks_new("iq");
		query = iks_new("query");
...
static int aji_ditems_handler(void *data, ikspak *pak)
{
	...
	if (!(node = iks_find_attrib(pak->query, "node"))) {
		...
		if (iq && query && item) {
			...
			iks_insert_attrib(item, "name", "Million Dollar GABpbx Commands");
	...
	case IKS_TYPE_SUBSCRIBE:
		if (ast_test_flag(&client->flags, AJI_AUTOACCEPT)) {
			...
			if (presence && status) {
				...
				iks_insert_cdata(status, "GABpbx has approved subscription", 0);
	...
	ASTOBJ_CONTAINER_TRAVERSE(&client->buddies, 1, {
		...
		if (ast_test_flag(&iterator->flags, AJI_AUTOPRUNE)) { /* If autoprune is set on jabber.conf */
			ast_aji_send(client, iks_make_s10n(IKS_TYPE_UNSUBSCRIBE, iterator->name,
								 "GoodBye. Your status is no longer needed by GABpbx the Open Source PBX"
								 " so I am no longer subscribing to your presence.\n"));
			ast_aji_send(client, iks_make_s10n(IKS_TYPE_UNSUBSCRIBED, iterator->name,
								 "GoodBye.  You are no longer in the GABpbx config file so I am removing"
...
static int aji_create_client(char *label, struct ast_variable *var, int debug)
{
	...
	*/
	...
	if (!strchr(client->user, '/') && !client->component) { /*client */
		resource = NULL;
		if (asprintf(&resource, "%s/gabpbx", client->user) >= 0) {
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "AJI - GABpbx Jabber Interface",
		.load = load_module,
