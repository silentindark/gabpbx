...
/* Colission between Net-SNMP and GABpbx */
										...
										int exact, size_t *var_len, WriteMethod **write_method)
{
	...
	case ASTCHANTYPENAME:
		*var_len = strlen(tech->type);
		/* T42.6: SNMP get-handler returns u_char *; tech->type is const char *
		 * but NetSNMP framework guarantees read-only on returned pointer.
		 * Union cast bypasses -Wcast-qual without UB. */
		{ union { const char *c; u_char *m; } u; u.c = tech->type; return u.m; }
	case ASTCHANTYPEDESC:
		*var_len = strlen(tech->description);
		{ union { const char *c; u_char *m; } u; u.c = tech->description; return u.m; }
							 ...
							 int exact, size_t *var_len, WriteMethod **write_method)
{
	...
	case ASTCONFSOCKET:
		*var_len = strlen(ast_config_AST_SOCKET);
		/* T42.6: see ASTCHANTYPENAME above for cast rationale */
		{ union { const char *c; u_char *m; } u; u.c = ast_config_AST_SOCKET; return u.m; }
							  ...
							  int exact, size_t *var_len, WriteMethod **write_method)
{
	...
	case ASTVERSTRING:
	{
		...
		*var_len = strlen(version);
		/* T42.6: see ASTCHANTYPENAME above for cast rationale */
		{ union { const char *c; u_char *m; } u; u.c = version; return u.m; }
	}
	case ASTVERTAG:
