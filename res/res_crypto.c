...
GABPBX_FILE_VERSION(__FILE__, "$Revision: 359110 $")

#include "gabpbx/paths.h"	/* use ast_config_AST_KEY_DIR */
...
static struct ast_key *try_load_key(const char *dir, const char *fname, int ifd, int ofd, int *not2)
{
	...
	/* T42.3: cast away const on strstr return — c is later written via *c='\0'
	 * (line 228) to truncate fname's extension. Caller try_load_key passes
	 * fname from d_name (writable readdir buffer) or tmp (writable strdupa
	 * in CLI handler) — both are writable, so the in-place truncation is safe. */
	...
	if (key->rsa) {
		...
		if (!notice) {
			if (!ast_opt_init_keys) {
				ast_log(LOG_NOTICE, "Add the '-i' flag to the gabpbx command line if you want to automatically initialize passcodes at launch.\n");
			}
			notice++;
		}
		/* Keep it anyway */
