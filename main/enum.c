...
*/
	...
	/* ISN rewrite */
	/* T42.3: number is const char * (ast_get_enum's API). ISN rewrite
	 * truncates it in place at '*'. Cast accepted because all in-tree
	 * callers (ast_get_enum invocations) pass either heap-allocated or
	 * stack buffers, never literals. The const in the API signature
	 * is the API-lie pattern. */
	if ((context->options & ENUMLOOKUP_OPTIONS_ISN) && (p1 = (char *)strchr(number, '*'))) {
		*p1++ = '\0';
		ast_copy_string(left, number, sizeof(left));
