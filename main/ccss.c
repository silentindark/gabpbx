...
GABPBX_FILE_VERSION(__FILE__, "$Revision: 369238 $")
...
static void *generic_recall(void *data)
{
	struct ast_cc_agent *agent = data;
	struct cc_generic_agent_pvt *generic_pvt = agent->private_data;
	/* T50: interface is always ast_strdupa'd from whichever S_OR branch picks
	 * (cc_params dialstring or device_name fallback). Writable for the strchr
	 * truncation below regardless of which input source is active. Replaces an
	 * earlier cast-with-runtime-invariant approach that assumed the dialstring
	 * field stays unused forever — fragile if a future cc_params consumer sets it. */
	const char *cc_dialstring = ast_get_cc_agent_dialstring(agent->cc_params);
	char *interface = ast_strdupa(S_OR(cc_dialstring, agent->device_name));
	const char *tech;
	char *target;
	int reason;
