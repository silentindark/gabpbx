...
static void handle_frame(struct ast_dial *dial, struct ast_dial_channel *channel, struct ast_frame *fr, struct ast_channel *chan)
{
	if (fr->frametype == AST_FRAME_CONTROL) {
		...
		case AST_CONTROL_ANSWER:
			if (channel->owner && channel->owner->cdr)
                                ast_verb(3, "[%s|%s] %s answered %s\n", channel->owner->cdr->clisrc, channel->owner->cdr->clidst, channel->owner->name, chan->name);
                        else
		...
		case AST_CONTROL_BUSY:
			if (channel->owner && channel->owner->cdr)
                                ast_verb(3, "[%s|%s] %s is busy\n", channel->owner->cdr->clisrc, channel->owner->cdr->clidst, channel->owner->name);
                        else
		...
		case AST_CONTROL_INCOMPLETE:
			if (channel->owner && channel->owner->cdr)
                                ast_verb(3, "[%s|%s] %s dialed Incomplete extension %s\n", channel->owner->cdr->clisrc, channel->owner->cdr->clidst, channel->owner->name, channel->owner->exten);
                        else
		...
		case AST_CONTROL_VIDUPDATE:
			if (channel->owner && channel->owner->cdr)
                                ast_verb(3, "[%s|%s] %s requested a video update, passing it to %s\n", channel->owner->cdr->clisrc, channel->owner->cdr->clidst, channel->owner->name, chan->name);
                        else
		...
		case AST_CONTROL_SRCUPDATE:
			if (option_verbose > 2) {
				if (channel->owner && channel->owner->cdr)
                                	ast_verb(3, "[%s|%s] %s requested a source update, passing it to %s\n", channel->owner->cdr->clisrc, channel->owner->cdr->clidst, channel->owner->name, chan->name);
                        	else
		...
		case AST_CONTROL_CONNECTED_LINE:
			if (channel->owner && channel->owner->cdr)
                                ast_verb(3, "[%s|%s] %s connected line has changed, passing it to %s\n", channel->owner->cdr->clisrc, channel->owner->cdr->clidst, channel->owner->name, chan->name);
                        else
		...
		case AST_CONTROL_REDIRECTING:
			if (channel->owner && channel->owner->cdr)
                                ast_verb(3, "[%s|%s] %s redirecting info has changed, passing it to %s\n", channel->owner->cdr->clisrc, channel->owner->cdr->clidst, channel->owner->name, chan->name);
                        else
		...
		case AST_CONTROL_HOLD:
			if (channel->owner && channel->owner->cdr)
                                ast_verb(3, "[%s|%s] Call on %s placed on hold\n", channel->owner->cdr->clisrc, channel->owner->cdr->clidst, chan->name);
                        else
		...
		case AST_CONTROL_UNHOLD:
			if (channel->owner && channel->owner->cdr)
                                ast_verb(3, "[%s|%s] Call on %s left from hold\n", channel->owner->cdr->clisrc, channel->owner->cdr->clidst, chan->name);
                        else
