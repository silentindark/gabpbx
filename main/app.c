			   ...
			   const char *restart, int skipms, long *offsetms)
{
	...
	/* T42.3: file is API-declared const char * but body needs to truncate at ':'.
	 * Cast accepted because all callers (res_agi argv[3], app_controlplayback
	 * args.filename via ast_strdupa, app_voicemail file param) pass writable
	 * buffers — verified by grep -rn 'ast_control_streamfile\b'. The const in
	 * the public signature is a long-standing API lie that would ripple
	 * through 3 callers + the prototype if dropped here. */
	if ((end = (char *)strchr(file, ':'))) {
