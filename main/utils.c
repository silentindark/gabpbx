...
GABPBX_FILE_VERSION(__FILE__, "$Revision: 378933 $")
...
#if !defined(HAVE_GETHOSTBYNAME_R_5) && !defined(HAVE_GETHOSTBYNAME_R_6) && !defined(__GLIBC__)
...
{
	char *e;
	const char *q;	/* T42.3: receives strchr(beg_quotes, ...) which is const char *; only read for offset (q - beg_quotes) and *q != '\0' */
