...
GABPBX_FILE_VERSION(__FILE__, "$Revision: 383165 $")
...
#endif
			/* OpenSSL 1.1.0+ handles thread cleanup automatically */
		}
...
static int __ssl_setup(struct ast_tls_config *cfg, int client)
{
	...
	if (client) {
		if (ast_test_flag(&cfg->flags, AST_SSL_TLSV1_CLIENT)) {
			...
			if (cfg->ssl_ctx) {
				SSL_CTX_set_min_proto_version(cfg->ssl_ctx, TLS1_VERSION);
				SSL_CTX_set_max_proto_version(cfg->ssl_ctx, TLS1_VERSION);
			}
		} else {
			cfg->ssl_ctx = SSL_CTX_new(TLS_client_method());
		}
	} else {
		cfg->ssl_ctx = SSL_CTX_new(TLS_server_method());
	}
	...
	/* TLS Hardening: disable SSLv2/SSLv3, enforce TLS 1.2 minimum */
	SSL_CTX_set_options(cfg->ssl_ctx,
	...
	if (!ast_strlen_zero(cfg->cipher)) {
		...
		/* Default secure cipher list when none configured */
		SSL_CTX_set_cipher_list(cfg->ssl_ctx,
			"ECDHE+AESGCM:ECDHE+CHACHA20:DHE+AESGCM:DHE+CHACHA20:!aNULL:!MD5:!RC4:!3DES:!DES");
	}
