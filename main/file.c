...
int ast_streamfile(struct ast_channel *chan, const char *filename, const char *preflang)
{
	...
	if (chan && chan->cdr)
                ast_verb(3, "[%s|%s] <%s> Playing '%s.%s' (language '%s')\n", chan->cdr->clisrc, chan->cdr->clidst, chan->name, filename, ast_getformatname(chan->writeformat), preflang ? preflang : "default");
        else
