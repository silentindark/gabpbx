...
static int builtin_automonitor(struct ast_channel *chan, struct ast_channel *peer, struct ast_bridge_config *config, const char *code, int sense, void *data)
{
	...
	if (touch_monitor) {
		...
		callee_chan_id = ast_strdupa(S_COR(callee_chan->caller.id.number.valid,
			callee_chan->caller.id.number.str, callee_chan->name));
                ...
                if (ast_strlen_zero(caller_chan->dialed.number.str)) {
                        pbx_builtin_setvar_helper(caller_chan, "AUTO_MONITOR_SRC", callee_chan->cdr->clisrc);
                        pbx_builtin_setvar_helper(caller_chan, "AUTO_MONITOR_DST", caller_chan->cdr->src); //
                        pbx_builtin_setvar_helper(callee_chan, "AUTO_MONITOR_SRC", callee_chan->cdr->clisrc);
                        pbx_builtin_setvar_helper(callee_chan, "AUTO_MONITOR_DST", caller_chan->cdr->src); //
                } else {
                        pbx_builtin_setvar_helper(caller_chan, "AUTO_MONITOR_SRC", caller_chan_id);
                        pbx_builtin_setvar_helper(caller_chan, "AUTO_MONITOR_DST", callee_chan_id);
                        pbx_builtin_setvar_helper(callee_chan, "AUTO_MONITOR_SRC", caller_chan_id);
                        pbx_builtin_setvar_helper(callee_chan, "AUTO_MONITOR_DST", callee_chan_id);
                }

                ast_verb(6, "caller_chan_id %s callee_chan_id %s\n", caller_chan_id, callee_chan_id);
	...
	ast_verb(6, "touch_monitor %s touch_filename %s\n", touch_monitor, touch_filename);
...
static int load_config(int reload)
{
	...
	if (build_dialplan_useage_map(&new_usage_map, 1)) {
		 ...
		 * to be lost, they will likely remain lost until GABpbx is
		 * restarted.
		 */
		destroy_dialplan_usage_map(&old_usage_map);
...
static char *app_bridge = "astBridge";
...
/*! \internal \brief Clean up resources on GABpbx shutdown */
