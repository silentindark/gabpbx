...
GABPBX_FILE_VERSION(__FILE__, "$Revision: 369869 $")
	...
	<application name="Answer" language="en_US">
		...
		<syntax>
			...
			<parameter name="nocdr">
				<para>GABpbx will send an answer signal to the calling phone, but will not
...
void ast_str_substitute_variables_full(struct ast_str **buf, ssize_t maxlen, struct ast_channel *c, struct varshead *headp, const char *templ, size_t *used)
{
	...
	while (!ast_strlen_zero(whereweare)) {
		...
		nextvar = NULL;
		nextexp = NULL;
		nextthing = (char *)strchr(whereweare, '$');	/* T42.3: whereweare is const char *cp1; cast OK because vars/vare derived later from nextvar+2 are passed to ast_expr/ast_func_read which take char * */
		if (nextthing) {
			switch (nextthing[1]) {
			case '{':
				nextvar = nextthing;
	...
	if (e) {
		...
		if (matching_action) {
			...
			if (VERBOSITY_ATLEAST(3)) {
				...
				if (c && c->cdr)
                                        ast_verb(3, "[%s|%s] Executing [%s@%s:%d] %s(\"%s\", \"%s\") %s\n",
                                        c->cdr->clisrc, c->cdr->clidst, exten, context, priority,
	...
	for (;;) {
		...
		if (found && res) {
			...
			if (strchr("0123456789ABCDEF*#", res)) {
				...
				if (c && c->cdr) {
                                        ast_debug(1, "[%s|%s] Spawn extension (%s,%s,%d) exited INCOMPLETE on '%s'\n", c->cdr->clisrc, c->cdr->clidst, c->context, c->exten, c->priority, c->name);
                                        ast_verb(2, "[%s|%s] Spawn extension (%s, %s, %d) exited INCOMPLETE on '%s'\n", c->cdr->clisrc, c->cdr->clidst, c->context, c->exten, c->priority, c->name);
                                } else {
				...
				if ((res == AST_PBX_ERROR)
					...
					if (!strcmp(c->exten, "e")) {
						if (c && c->cdr)
                                                        ast_verb(2, "[%s|%s] Spawn extension (%s, %s, %d) exited ERROR while already on 'e' exten on '%s'\n", c->cdr->clisrc, c->cdr->clidst, c->context, c->exten, c->priority, c->name);
                                                else
...
static int pbx_builtin_goto(struct ast_channel *chan, const char *data)
{
	...
	if (!res)
		ast_verb(3, "[%s|%s] Goto (%s,%s,%d)\n", chan->cdr->clisrc, chan->cdr->clidst, chan->context, chan->exten, chan->priority + 1);
	return res;
}
...
static const struct ast_data_entry pbx_data_providers[] = {
	AST_DATA_ENTRY("gabpbx/core/hints", &hints_data_provider),
};
...
static int pbx_parseable_goto(struct ast_channel *chan, const char *goto_string, int async)
{
	...
	if (ast_strlen_zero(goto_string)) {
		ast_log(LOG_WARNING, "[%s|%s] Goto requires an argument ([[context,]extension,]priority)\n", chan->cdr->clisrc, chan->cdr->clidst);
		return -1;
	}
