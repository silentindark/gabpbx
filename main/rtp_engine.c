...
void ast_rtp_codecs_payloads_clear(struct ast_rtp_codecs *codecs, struct ast_rtp_instance *instance)
{
	...
	for (i = 0; i < AST_RTP_MAX_PT; i++) {
		codecs->payloads[i].gabpbx_format = 0;
...
void ast_rtp_codecs_payloads_default(struct ast_rtp_codecs *codecs, struct ast_rtp_instance *instance)
{
	...
	for (i = 0; i < AST_RTP_MAX_PT; i++) {
		if (static_RTP_PT[i].code) {
			codecs->payloads[i].gabpbx_format = static_RTP_PT[i].gabpbx_format;
			codecs->payloads[i].code = static_RTP_PT[i].code;
			if (instance && instance->engine && instance->engine->payload_set) {
				instance->engine->payload_set(instance, i, codecs->payloads[i].gabpbx_format, codecs->payloads[i].code);
			}
		}
	}
}
...
void ast_rtp_codecs_payloads_copy(struct ast_rtp_codecs *src, struct ast_rtp_codecs *dest, struct ast_rtp_instance *instance)
{
	...
	for (i = 0; i < AST_RTP_MAX_PT; i++) {
		if (src->payloads[i].code) {
			...
			dest->payloads[i].gabpbx_format = src->payloads[i].gabpbx_format;
			dest->payloads[i].code = src->payloads[i].code;
			if (instance && instance->engine && instance->engine->payload_set) {
				instance->engine->payload_set(instance, i, dest->payloads[i].gabpbx_format, dest->payloads[i].code);
			}
		}
	}
}
...
void ast_rtp_codecs_payloads_set_m_type(struct ast_rtp_codecs *codecs, struct ast_rtp_instance *instance, int payload)
{
	...
	codecs->payloads[payload].gabpbx_format = static_RTP_PT[payload].gabpbx_format;
	codecs->payloads[payload].code = static_RTP_PT[payload].code;

	ast_debug(1, "Setting payload %d based on m type on %p\n", payload, codecs);

	if (instance && instance->engine && instance->engine->payload_set) {
		instance->engine->payload_set(instance, payload, codecs->payloads[payload].gabpbx_format, codecs->payloads[payload].code);
	}
}
				 ...
				 unsigned int sample_rate)
{
	...
	for (i = 0; i < ARRAY_LEN(ast_rtp_mime_types); ++i) {
		...
		if ((t->payload_type.code == AST_FORMAT_G726) &&
		                        t->payload_type.gabpbx_format &&
		    (options & AST_RTP_OPT_G726_NONSTANDARD)) {
			codecs->payloads[pt].code = AST_FORMAT_G726_AAL2;
		}

		if (instance && instance->engine && instance->engine->payload_set) {
			instance->engine->payload_set(instance, pt, codecs->payloads[i].gabpbx_format, codecs->payloads[i].code);
		}
...
void ast_rtp_codecs_payloads_unset(struct ast_rtp_codecs *codecs, struct ast_rtp_instance *instance, int payload)
{
	...
	codecs->payloads[payload].gabpbx_format = 0;
...
{
	struct ast_rtp_payload_type result = { .gabpbx_format = 0, };

	if (payload < 0 || payload >= AST_RTP_MAX_PT) {
		return result;
	}

	result.gabpbx_format = codecs->payloads[payload].gabpbx_format;
...
void ast_rtp_codecs_payload_formats(struct ast_rtp_codecs *codecs, format_t *astformats, int *nonastformats)
{
	...
	*astformats = *nonastformats = 0;
	...
	for (i = 0; i < AST_RTP_MAX_PT; i++) {
		...
		if (codecs->payloads[i].gabpbx_format) {
...
int ast_rtp_codecs_payload_code(struct ast_rtp_codecs *codecs, const int gabpbx_format, const format_t code)
{
	int i;

	for (i = 0; i < AST_RTP_MAX_PT; i++) {
		if (codecs->payloads[i].gabpbx_format == gabpbx_format && codecs->payloads[i].code == code) {
			return i;
		}
	}

	for (i = 0; i < AST_RTP_MAX_PT; i++) {
		if (static_RTP_PT[i].gabpbx_format == gabpbx_format && static_RTP_PT[i].code == code) {
			return i;
		}
	}
...
const char *ast_rtp_lookup_mime_subtype2(const int gabpbx_format, const format_t code, enum ast_rtp_options options)
{
	int i;

	for (i = 0; i < ARRAY_LEN(ast_rtp_mime_types); i++) {
		if (ast_rtp_mime_types[i].payload_type.code == code && ast_rtp_mime_types[i].payload_type.gabpbx_format == gabpbx_format) {
			if (gabpbx_format && (code == AST_FORMAT_G726_AAL2) && (options & AST_RTP_OPT_G726_NONSTANDARD)) {
				return "G726-32";
			} else {
...
unsigned int ast_rtp_lookup_sample_rate2(int gabpbx_format, format_t code)
{
	unsigned int i;
...
char *ast_rtp_lookup_mime_multiple2(struct ast_str *buf, const format_t capability, const int gabpbx_format, enum ast_rtp_options options)
{
	format_t format;
	int found = 0;
	...
	for (format = 1; format < AST_RTP_MAX; format <<= 1) {
		if (capability & format) {
			const char *name = ast_rtp_lookup_mime_subtype2(gabpbx_format, format, options);
...
format_t ast_rtp_instance_available_formats(struct ast_rtp_instance *instance, format_t to_endpoint, format_t to_gabpbx)
{
	format_t formats;

	if (instance->engine->available_formats && (formats = instance->engine->available_formats(instance, to_endpoint, to_gabpbx))) {
		return formats;
	}

	return ast_translate_available_formats(to_endpoint, to_gabpbx);
}
