...
GABPBX_FILE_VERSION(__FILE__, "$Revision: 388195 $")
...
/*! \brief Returns non-zero if GABpbx is being shut down */
...
__ast_channel_alloc_ap(int needqueue, int state, const char *cid_num, const char *cid_name,
		       ...
		       const char *linkedid, const int amaflag, const char *remote_uniqueid, const char *file, int line,
		       const char *function, const char *name_fmt, va_list ap1, va_list ap2)
{
	...
	if (!ast_strlen_zero(remote_uniqueid))
		ast_string_field_set(tmp, remote_uniqueid, remote_uniqueid);
					...
					const char *name_fmt, ...)
{
	...
	result = __ast_channel_alloc_ap(needqueue, state, cid_num, cid_name, acctcode, exten, context,
					linkedid, amaflag, NULL, file, line, function, name_fmt, ap1, ap2);
	va_end(ap1);
	va_end(ap2);
...
struct ast_channel *__ast_channel_alloc_remote_uniqueid(int needqueue, int state, const char *cid_num,
                                        const char *cid_name, const char *acctcode,
                                        const char *exten, const char *context,
                                        const char *linkedid, const int amaflag, const char *remote_uniqueid,
                                        const char *file, int line, const char *function,
                                        const char *name_fmt, ...)
{
        va_list ap1, ap2;
        ...
        va_start(ap2, name_fmt);
        result = __ast_channel_alloc_ap(needqueue, state, cid_num, cid_name, acctcode, exten, context,
                                        linkedid, amaflag, remote_uniqueid, file, line, function, name_fmt, ap1, ap2);
        va_end(ap1);
        va_end(ap2);
	...
	if (f) {
		...
		*/
		...
		case AST_FRAME_DTMF_END:
			...
			ast_log(LOG_DTMF, "[%s|%s] DTMF end '%c' received on %s, duration %ld ms\n", chan->cdr->clisrc, chan->cdr->clidst, f->subclass.integer, chan->name, f->len);
			...
			if (ast_test_flag(chan, AST_FLAG_DEFER_DTMF) || ast_test_flag(chan, AST_FLAG_EMULATE_DTMF)) {
				...
				if (!ast_tvzero(chan->dtmf_tv) && 
					...
					ast_log(LOG_DTMF, "[%s|%s] DTMF begin emulation of '%c' with duration %u queued on %s\n", chan->cdr->clisrc, chan->cdr->clidst, f->subclass.integer, chan->emulate_dtmf_duration, chan->name);
				}
				if (chan->audiohooks) {
				...
				if (ast_test_flag(chan, AST_FLAG_IN_DTMF)) {
					ast_log(LOG_DTMF, "[%s|%s] DTMF end accepted with begin '%c' on %s\n", chan->cdr->clisrc, chan->cdr->clidst, f->subclass.integer, chan->name);
					...
					if (ast_tvdiff_ms(now, chan->dtmf_tv) < AST_MIN_DTMF_DURATION) {
						f->len = ast_tvdiff_ms(now, chan->dtmf_tv);
						ast_log(LOG_DTMF, "[%s|%s] DTMF end '%c' detected to have actual duration %ld on the wire, emulation will be triggered on %s\n", chan->cdr->clisrc, chan->cdr->clidst, f->subclass.integer, f->len, chan->name);
					}
				} else if (!f->len) {
					ast_log(LOG_DTMF, "[%s|%s] DTMF end accepted without begin '%c' on %s\n", chan->cdr->clisrc, chan->cdr->clidst, f->subclass.integer, chan->name);
					f->len = AST_MIN_DTMF_DURATION;
				}
				if (f->len < AST_MIN_DTMF_DURATION && !ast_test_flag(chan, AST_FLAG_END_DTMF_ONLY)) {
					ast_log(LOG_DTMF, "[%s|%s] DTMF end '%c' has duration %ld but want minimum %d, emulating on %s\n", chan->cdr->clisrc, chan->cdr->clidst, f->subclass.integer, f->len, AST_MIN_DTMF_DURATION, chan->name);
					ast_set_flag(chan, AST_FLAG_EMULATE_DTMF);
					chan->emulate_dtmf_digit = f->subclass.integer;
					...
					ast_log(LOG_DTMF, "[%s|%s] DTMF end passthrough '%c' on %s\n", chan->cdr->clisrc, chan->cdr->clidst, f->subclass.integer, chan->name);
					if (f->len < AST_MIN_DTMF_DURATION) {
						f->len = AST_MIN_DTMF_DURATION;
					}
					chan->dtmf_tv = now;
		...
		case AST_FRAME_DTMF_BEGIN:
			...
			ast_log(LOG_DTMF, "[%s|%s] DTMF begin '%c' received on %s\n", chan->cdr->clisrc, chan->cdr->clidst, f->subclass.integer, chan->name);
			if ( ast_test_flag(chan, AST_FLAG_DEFER_DTMF | AST_FLAG_END_DTMF_ONLY | AST_FLAG_EMULATE_DTMF) || 
			    (!ast_tvzero(chan->dtmf_tv) && 
			      ast_tvdiff_ms(ast_tvnow(), chan->dtmf_tv) < AST_MIN_DTMF_GAP) ) {
				ast_log(LOG_DTMF, "[%s|%s] DTMF begin ignored '%c' on %s\n", chan->cdr->clisrc, chan->cdr->clidst, f->subclass.integer, chan->name);
				ast_frfree(f);
				f = &ast_null_frame;
			} else {
				ast_set_flag(chan, AST_FLAG_IN_DTMF);
				chan->dtmf_tv = ast_tvnow();
				ast_log(LOG_DTMF, "[%s|%s] DTMF begin passthrough '%c' on %s\n", chan->cdr->clisrc, chan->cdr->clidst, f->subclass.integer, chan->name);
			}
			break;
		case AST_FRAME_NULL:
			...
			if (ast_test_flag(chan, AST_FLAG_EMULATE_DTMF)) {
				...
				if (!chan->emulate_dtmf_duration) {
					...
					ast_clear_flag(chan, AST_FLAG_EMULATE_DTMF);
					chan->emulate_dtmf_digit = 0;
					ast_log(LOG_DTMF, "[%s|%s] DTMF end emulation of '%c' queued on %s\n", chan->cdr->clisrc, chan->cdr->clidst, f->subclass.integer, chan->name);
					if (chan->audiohooks) {
						struct ast_frame *old_frame = f;
						f = ast_audiohook_write_list(chan, chan->audiohooks, AST_AUDIOHOOK_DIRECTION_READ, f);
						if (old_frame != f) {
...
/*!
 * \internal
 * \brief /gabpbx/core/channels provider.
 */
...
/*!
 * \internal
 * \brief /gabpbx/core/channeltypes provider.
 */
...
static const struct ast_data_entry channel_providers[] = {
	AST_DATA_ENTRY("/gabpbx/core/channels", &channels_provider),
	AST_DATA_ENTRY("/gabpbx/core/channeltypes", &channeltypes_provider),
};
				      ...
				      const char *name_fmt, ...)
{
	...
	va_start(ap2, name_fmt);
	result = __ast_channel_alloc_ap(needqueue, state, cid_num, cid_name, acctcode, exten, context,
					linkedid, amaflag, NULL, __FILE__, __LINE__, __FUNCTION__, name_fmt, ap1, ap2);
	va_end(ap1);
	va_end(ap2);
