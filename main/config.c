		...
		*last_cat = newcat;
		...
		if (!strcasecmp(cur, "include")) {
			...
			if (!ast_opt_exec_includes) {
				ast_log(LOG_WARNING, "Cannot perform #exec unless execincludes option is enabled in gabpbx.conf (options section)!\n");
...
int read_config_maps(void) 
{
	...
	for (v = ast_variable_browse(config, "settings"); v; v = v->next) {
		...
		if (!strcmp(v->name, "gabpbx.conf")) {
			ast_log(LOG_WARNING, "Cannot bind 'gabpbx.conf'!\n");
			continue;
		}
