...
static struct logchannel *make_logchannel(const char *channel, const char *components, int lineno)
{
	...
	const char *facility;	/* T42.3: receives strchr(channel, ...) const result; passed read-only to ast_syslog_facility */
...
char **ast_bt_get_symbols(void **addresses, size_t num_frames)
{
	...
	for (stackfr = 0; stackfr < num_frames; stackfr++) {
		...
		if (strcmp(dli.dli_fname, "gabpbx") == 0) {
			char gabpbxpath[256];
			if (!(dli.dli_fname = ast_utils_which("gabpbx", gabpbxpath, sizeof(gabpbxpath)))) {
				/* This will fail to find symbols */
				ast_log(LOG_DEBUG, "Failed to find gabpbx binary for debug symbols.\n");
				dli.dli_fname = "gabpbx";
			}
		}
