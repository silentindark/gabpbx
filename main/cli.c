...
#define VERBOSE_FORMAT_STRING2 "%-20.20s %-20.20s %-16.16s %-4.4s %-7.7s %-12.12s %-25.25s %-15.15s %8.8s %-11.11s %-11.11s %-20.20s\n"
	...
	int numchans = 0, concise = 0, verbose = 0, count = 0, number = 0;
	struct ast_channel_iterator *iter = NULL;

	switch (cmd) {
	case CLI_INIT:
		e->command = "core show channels [concise|verbose|count|number]";
		e->usage =
			"Usage: core show channels [concise|verbose|count|number]\n"
			...
			"	of GABpbx.\n"
			"	If 'number' only shows active channels count\n";
		return NULL;
	...
	if (a->argc == e->args) {
		if (!strcasecmp(a->argv[e->args-1],"concise"))
			...
			count = 1;
		else if (!strcasecmp(a->argv[e->args-1],"number"))
			number = 1;
		else
			return CLI_SHOWUSAGE;
	} else if (a->argc != e->args - 1)
...
char *ast_complete_channels(const char *line, const char *word, int pos, int state, int rpos)
{
	...
	while (!found && (c = ast_channel_iterator_next(iter))) {
		if (++which > state) {
			ast_channel_lock(c);
