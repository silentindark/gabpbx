...
GABPBX_FILE_VERSION(__FILE__, "$Revision: 369366 $")
...
void ast_cdr_getvar(struct ast_cdr *cdr, const char *name, char **ret, char *workspace, int workspacelen, int recur, int raw)
{
	...
	*ret = NULL;
	...
	else if (!strcasecmp(name, "callfrom"))
		snprintf(workspace, workspacelen, "%i", cdr->callfrom);
...
static const char * const cdr_readonly_vars[] = { "clid", "src", "dst", "dcontext", "channel", "dstchannel",
						  ...
						  "billsec", "disposition", "amaflags", "accountcode", "uniqueid", "remote_uniqueid", "linkedid",
						  "userfield", "clisrc", "clidst", "sequence", "callfrom", NULL };
...
void ast_cdr_merge(struct ast_cdr *to, struct ast_cdr *from)
{
	...
	if (ast_strlen_zero(to->clisrc) && !ast_strlen_zero(from->clisrc)) {
                ast_copy_string(to->clisrc, from->clisrc, sizeof(to->clisrc));
        }
	if (ast_strlen_zero(to->clidst) && !ast_strlen_zero(from->clidst)) {
                ast_copy_string(to->clidst, from->clidst, sizeof(to->clidst));
        }

        to->callfrom = from->callfrom;
...
static void set_one_cid(struct ast_cdr *cdr, struct ast_channel *c)
{
	...
	ast_copy_string(cdr->clisrc, S_OR(num, ""), sizeof(cdr->clisrc));
...
int ast_cdr_init(struct ast_cdr *cdr, struct ast_channel *c)
{
	for ( ; cdr ; cdr = cdr->next) {
		if (!ast_test_flag(cdr, AST_CDR_FLAG_LOCKED)) {
			...
			ast_copy_string(cdr->remote_uniqueid, c->remote_uniqueid, sizeof(cdr->remote_uniqueid));
			/* Linked call identifier */
			ast_copy_string(cdr->linkedid, c->linkedid, sizeof(cdr->linkedid));
			ast_copy_string(cdr->clidst  , S_OR(c->macroexten,c->exten), sizeof(cdr->clidst));
		}
	}
	return 0;
}
...
static void post_cdr(struct ast_cdr *cdr)
{
	...
	for ( ; cdr ; cdr = cdr->next) {
		 ...
		 * channels were originated from gabpbx (pbx_spool, manager,
		 * cli) */
