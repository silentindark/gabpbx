...
/* GTK2 path: GtkItemFactory drives the menubar (removed in GTK3). */
...
#if GTK_CHECK_VERSION(3,0,0)
/* GTK3+ path: GtkItemFactory was removed; build the menu directly using
 * GtkMenuBar + GtkMenu + GtkMenuItem. Stock items also gone — use literal
 * labels. Functionally identical: File > Save And Quit / Quit; Help > About. */
static GtkWidget *get_menubar_menu(GtkWidget *window)
{
	GtkAccelGroup *accel_group;
	GtkWidget *menubar, *file_menu, *help_menu;
	GtkWidget *file_item, *help_item;
	GtkWidget *save_item, *sep_item, *quit_item, *about_item;
	...
	save_item = gtk_menu_item_new_with_mnemonic("_Save And Quit");
	gtk_widget_add_accelerator(save_item, "activate", accel_group,
		GDK_KEY_s, GDK_CONTROL_MASK, GTK_ACCEL_VISIBLE);
	g_signal_connect(save_item, "activate", G_CALLBACK(handle_save), NULL);
	...
	g_signal_connect(about_item, "activate", G_CALLBACK(handle_about), NULL);
	gtk_menu_shell_append(GTK_MENU_SHELL(help_menu), about_item);

	return menubar;
}
#else
/* GTK2 path: original GtkItemFactory implementation (the API was removed in GTK3). */
static GtkWidget *get_menubar_menu(GtkWidget *window)
{
