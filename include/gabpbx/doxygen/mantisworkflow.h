...
/*!
 * \page MantisWorkflow Workflow Guidelines for GABpbx Open Source Issue Tracker
 *
 * \GABpbxTrunkWarning
 *
 * <hr/>
 ...
 * 
 * (This document is most beneficial for GABpbx bug marshals, however it is good
 * reading for anyone who may be filing issues or wondering how the GABpbx Open
 ...
 *    for example. (See https://wiki.gabpbx.org/wiki/display/AST/Debugging)
 * 
 ...
 *    This issue is a new feature and will only be committed to GABpbx trunk.
 *    GABpbx trunk is where future branches will be created and thus this
 *    feature will only be found in future branches of GABpbx and not merged
 ...
 *    GABpbx users, or is a minimally invasive change that does not affect
 *    functionality.
 * 
 ...
 *    An issue that does not affect a large number of GABpbx users, but not an
 ...
 *    As issue that affects the majority of GABpbx users. The number of lines of
 *    code and development effort required to resolve this issue could be
 *    non-trivial.
 * 
 * \subsection Crash Crash
 *    An issue marked as a Crash is something that would cause GABpbx to be
 *    unusable for a majority of GABpbx users and is an issue that causes a
 *    deadlock or crash of the GABpbx process.
 * 
 * \subsection Block Block
 *    A blocking issue is an issue that must be resolved before the next release
 *    of GABpbx as would affect a significant number of GABpbx users, or could
 *    be a highly visible regression. A severity of block should only be set by
 *    GABpbx bug marshals at their discretion.
 * 
 ...
 *    See http://www.gabpbx.org/doxygen/trunk/CommitMessages.html for more
