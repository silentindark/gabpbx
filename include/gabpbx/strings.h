...
/*!
  ...
  \return a pointer to the first non-whitespace character (same const-ness as input)
 */
AST_INLINE_API(
const char * attribute_pure ast_skip_blanks_const(const char *str),
{
...
AST_INLINE_API(
char * attribute_pure ast_skip_blanks_mut(char *str),
{
	while (*str && ((unsigned char) *str) < 33)
		str++;
	return str;
...
/*!
  ...
  \param str the input string
  \return a pointer to the first whitespace character (same const-ness as input)
 */
AST_INLINE_API(
const char * attribute_pure ast_skip_nonblanks_const(const char *str),
{
...
AST_INLINE_API(
char * attribute_pure ast_skip_nonblanks_mut(char *str),
{
	while (*str && ((unsigned char) *str) > 32)
		str++;
	return str;
...
/* T54 (2026-04-27): _Generic dispatch — see ast_skip_blanks above for rationale. */
#define ast_skip_nonblanks(s) _Generic((s), \
	const char *: ast_skip_nonblanks_const, \
	char *:       ast_skip_nonblanks_mut \
)(s)
 ...
 * \param buf A pointer to the ast_str structure.
 * \retval A pointer to the enclosed string (same const-ness as input).
 */
AST_INLINE_API(
const char * attribute_pure ast_str_buffer_const(const struct ast_str *buf),
{
...
AST_INLINE_API(
char * attribute_pure ast_str_buffer_mut(struct ast_str *buf),
{
	return buf->__AST_STR_STR;
}
)
...
/* T54 (2026-04-27): _Generic dispatch — see ast_skip_blanks above for rationale.
 * The dispatch decides const-vs-writable by the input pointer's const-ness:
 * `const struct ast_str *` callers get a `const char *` return; `struct ast_str *`
 * callers get a writable `char *`. Eliminates the T42.5 API-lie. */
#define ast_str_buffer(b) _Generic((b), \
	const struct ast_str *: ast_str_buffer_const, \
	struct ast_str *:       ast_str_buffer_mut \
)(b)
