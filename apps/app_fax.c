 ...
 *
 * Copyright (C) 2007-2008, Dmitry Andrianov <gabpbx@dima.spb.ru>
 *
...
GABPBX_FILE_VERSION(__FILE__, "$Revision: 340108 $")
...
/* Setup SPAN logging according to GABpbx debug level */
...
static int transmit(fax_session *s)
{
	...
	if (s->chan->_state != AST_STATE_UP) {
		 ...
		 * Theoretically gabpbx should already have answered before running the app */
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "Simple FAX Application",
		.load = load_module,
