...
static int do_say(say_args_t *a, const char *s, const char *options, int depth)
{
	...
	/* T42.3: s is const char *; we only use x here for offset arithmetic
	 * (s = x + 1), no writes through it. x is reassigned writable later via
	 * strsep + ast_skip_blanks (lines 217 + 222), so must remain char *. */
	if ( (x = (char *)strchr(s, ':')) )
		s = x + 1;
	if ( (x = (char *)strchr(s, ':')) )
		s = x + 1;
