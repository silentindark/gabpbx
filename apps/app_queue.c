...
GABPBX_FILE_VERSION(__FILE__, "$Revision: 369262 $")
...
static void queue_set_param(struct call_queue *q, const char *param, const char *val, int linenum, int failunknown)
{
	if (!strcasecmp(param, "musicclass") || 
		...
		if (strategy == QUEUE_STRATEGY_LINEAR) {
			ast_log(LOG_WARNING, "Changing to the linear strategy currently requires gabpbx to be restarted.\n");
			return;
		}
		q->strategy = strategy;
...
static const struct ast_data_entry queue_data_providers[] = {
	AST_DATA_ENTRY("gabpbx/application/queue/list", &queues_data_provider),
};
