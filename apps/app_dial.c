...
GABPBX_FILE_VERSION(__FILE__, "$Revision: 369258 $")
	...
	struct ast_party_id *forced_clid, struct ast_party_id *stored_clid)
{
	...
	if (ast_test_flag64(peerflags, OPT_IGNORE_FORWARDING)) {
		...
		if (c) {
			...
			if (!ast_strlen_zero(o->chan->accountcode)) {
				ast_string_field_set(in, accountcode, o->chan->accountcode);
				ast_verb(3, "Now forwarding %s to '%s/%s' accountcode %s\n", in->name, tech, stuff, in->accountcode);
			}
	...
	while (*to && !peer) {
		...
		if (pos == 1) { /* only the input channel is available */
			if (numlines == (num.busy + num.congestion + num.nochan)) {
                                if (in && in->cdr)
                                        ast_verb(2, "[%s|%s] Everyone is busy/congested at this time (%d:%d/%d/%d)\n", in->cdr->clisrc, in->cdr->clidst, numlines, num.busy, num.congestion, num.nochan);
                                else
				...
				if (num.busy)
					...
					strcpy(pa->status, "CHANUNAVAIL");
                                ...
                                if (in && in->cdr)
                                        ast_verb(3, "[%s|%s] No one is available to answer at this time (%d:%d/%d/%d)\n", in->cdr->clisrc, in->cdr->clidst, numlines, num.busy, num.congestion, num.nochan);
                                else
			...
			*to = 0;
			if (is_cc_recall) {
                                if (in && in->cdr)
                                        ast_cc_failed(cc_recall_core_id, "[%s|%s] Everyone is busy/congested for the recall. How sad", in->cdr->clisrc, in->cdr->clidst);
                                else
		...
		for (o = outgoing; o; o = o->next) {
			...
			if (ast_test_flag64(o, DIAL_STILLGOING) && c->_state == AST_STATE_UP) {
				if (!peer) {
                                        if (in && in->cdr)
                                                ast_verb(3, "[%s|%s] %s answered %s\n", in->cdr->clisrc, in->cdr->clidst, c->name, in->name);
                                        else
			...
			case AST_FRAME_CONTROL:
				...
				case AST_CONTROL_BUSY:
                                        if (in && in->cdr)
                                                ast_verb(3, "[%s|%s] %s is busy\n", in->cdr->clisrc, in->cdr->clidst, c->name);
                                        else
				...
				case AST_CONTROL_CONGESTION:
                                        if (in && in->cdr)
                                                ast_verb(3, "[%s|%s] %s is circuit-busy\n", in->cdr->clisrc, in->cdr->clidst, c->name);
                                        else
				...
				case AST_CONTROL_PROGRESS:
                                        if (in && in->cdr)
                                                ast_verb(3, "[%s|%s] %s is making progress passing it to %s\n", in->cdr->clisrc, in->cdr->clidst, c->name, in->name);
                                        else
				...
				case AST_CONTROL_PROCEEDING:
                                        if (in && in->cdr)
                                                ast_verb(3, "[%s|%s] %s is proceeding passing it to %s\n", in->cdr->clisrc, in->cdr->clidst, c->name, in->name);
                                        else
...
skip_frame:;
		...
		if (!*to) { /* germanico */
			...
			in->hangupcause = AST_CAUSE_RECOVERY_ON_TIMER_EXPIRE;
		}
...
static int dial_exec_full(struct ast_channel *chan, const char *data, struct ast_flags64 *peerflags, int *continue_exec)
{
	...
	while ((cur = strsep(&rest, "&")) ) {
		...
		pbx_builtin_setvar_helper(tc, "CLIBILLING", pbx_builtin_getvar_helper(chan, "CLIBILLING"));
		...
		if (res) {
			...
			senddialevent(chan, tc, numsubst);
                        if (chan && chan->cdr)
                                ast_verb(3, "[%s|%s] Called %s/%s\n", chan->cdr->clisrc, chan->cdr->clidst, tech, numsubst);
                        else
	...
	if (!peer) {
		...
		strcpy(chan->cdr->dialedpeernumber, number);
		...
		if (ast_test_flag64(&opts, OPT_PEER_H)
			...
			if (found && res9) {
				/* Something bad happened, or a hangup has been requested. */
                                if (chan && chan->cdr) {
                                        ast_debug(1, "[%s|%s] Spawn extension (%s,%s,%d) exited non-zero on '%s'\n", chan->cdr->clisrc, chan->cdr->clidst, peer->context, peer->exten, peer->priority, peer->name);
                                        ast_verb(2, "[%s|%s] Spawn extension (%s, %s, %d) exited non-zero on '%s'\n", chan->cdr->clisrc, chan->cdr->clidst, peer->context, peer->exten, peer->priority, peer->name);
                                } else{
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Dialing Application");
