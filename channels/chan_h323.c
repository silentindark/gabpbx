...
/*! \file
 *
 * \brief This file is part of the chan_h323 driver for GABpbx
 *
...
GABPBX_FILE_VERSION(__FILE__, "$Revision: 350789 $")
...
static struct oh323_pvt {
	...
	int needhangup;				/*!< Send hangup when GABpbx is ready */
	...
	int curDTMF;				/*!< DTMF tone being generated to GABpbx side */
...
static void __oh323_update_info(struct ast_channel *c, struct oh323_pvt *pvt)
{
	...
	if (pvt->update_rtp_info > 0) {
		if (pvt->rtp) {
			...
			ast_queue_frame(pvt->owner, &ast_null_frame);	/* Tell GABpbx to apply changes */
		}
...
static int __oh323_rtp_create(struct oh323_pvt *pvt)
{
	...
	pvt->rtp = ast_rtp_instance_new("gabpbx", sched, &our_addr, NULL);
	if (!pvt->rtp) {
		ast_mutex_unlock(&pvt->lock);
...
/*! \brief
  * Call-back function passing remote ip/port information from H.323 to gabpbx
  *
  * Returns nothing
  */
...
/*! \brief
  *	Call-back function to signal gabpbx that the channel has been answered
  * Returns nothing
  */
static void connection_made(unsigned call_reference, const char *token)
{
	...
	/* Inform gabpbx about remote party connected only on outgoing calls */
	if (!pvt->outgoing) {
		ast_mutex_unlock(&pvt->lock);
		return;
	}
...
static int progress(unsigned call_reference, const char *token, int inband)
{
	...
	if (!pvt->owner) {
		ast_mutex_unlock(&pvt->lock);
		ast_log(LOG_ERROR, "No GABpbx channel associated with private structure.\n");
		return -1;
	}
...
static int answer_call(unsigned call_reference, const char *token)
{
	...
	if (h323debug)
		ast_debug(1, "Preparing GABpbx to answer for %s\n", token);
	...
	/* allocate a channel and tell gabpbx about it */
...
/*! \brief
  *  Call-back function to signal gabpbx that the channel is ringing
  *  Returns nothing
  */
...
static int reload_config(int is_reload)
{
	...
	for (v = ast_variable_browse(cfg, "general"); v; v = v->next) {
		...
		if (!strcasecmp(v->name, "port")) {
			...
			ast_log(LOG_WARNING, "The \"tos\" setting is deprecated in this version of GABpbx. Please change to \"tos_audio\".\n");
			...
			ast_log(LOG_WARNING, "The \"cos\" setting is deprecated in this version of GABpbx. Please change to \"cos_audio\".\n");
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "The NuFone Network's OpenH323 Channel Driver",
		.load = load_module,
