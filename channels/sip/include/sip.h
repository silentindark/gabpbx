 ...
 * There is a limited number of places in gabpbx where we could,
 * in principle, use a different "default" port number, but
 * we do not support this feature at the moment.
 * You can run GABpbx with SIP on a different port with a configuration
...
#define DEFAULT_VMEXTEN        "gabpbx" /*!< Default voicemail extension */
#define DEFAULT_CALLERID       "gabpbx" /*!< Default caller ID */
...
#define DEFAULT_REALM          "gabpbx" /*!< Realm for HTTP digest authentication */
...
#define DEFAULT_USERAGENT  "GABpbx PBX"  /*!< Default Useragent: header unless re-defined in sip.conf */
#define DEFAULT_SDPSESSION "GABpbx PBX"  /*!< Default SDP session name, (s=) header unless re-defined in sip.conf */
#define DEFAULT_SDPOWNER   "root"          /*!< Default SDP username field in (o=) header unless re-defined in sip.conf */
#define DEFAULT_ENGINE     "gabpbx"      /*!< Default RTP engine to use for sessions */
 ...
 *  \note Endpoints, like GABpbx, should always use WWW authentication to
...
/*! \brief Modes in which GABpbx can be configured to run SIP Session-Timers */
enum st_mode {
...
enum st_refresher {
        ...
        SESSION_TIMER_REFRESHER_US,   /*!< Initially prefer session refresh by GABpbx */
...
/*! \brief Define some implemented SIP transports
	\note GABpbx does not support SCTP or UDP/DTLS
*/
enum sip_transport {
...
/*! \brief States whether a SIP message can create a dialog in GABpbx. */
...
/*! \brief SIP Request methods known by GABpbx
 *
...
enum sipmethod {
	...
	SIP_PRACK,      /*!< Reliable pre-call signalling. Not supported in GABpbx. */
	...
	SIP_PUBLISH,    /*!< Not supported in GABpbx */
...
struct sip_pvt {
	...
	AST_DECLARE_STRING_FIELDS(
		...
		AST_STRING_FIELD(sipserver);    /*!< SIP server */
	 ...
	 *
	 * The reason for the length being 4 (OFFERED_MEDIA_COUNT) is that in this branch of GABpbx, the only media types supported are
...
//		AST_STRING_FIELD(lastfullcontact); /*!< Last Contact registered with us (not in sip.conf) */
	...
	unsigned short host_dynamic:1;  /*!< Dynamic Peers register with GABpbx */
	...
	struct ast_sockaddr lastaddr;   /*!< Last IP address of peer */
	unsigned short lockuseragent:1;
 ...
 * implementation in GABpbx conforms to the defintions there. An EPA is a UAC that
 ...
 *
 * A sip_esc_entry corresponds to an entity which has sent a PUBLISH to GABpbx. We are
...
static const struct cfsip_options {
	int id;             /*!< Bitmap ID */
	int supported;      /*!< Supported by GABpbx ? */
