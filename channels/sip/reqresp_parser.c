...
GABPBX_FILE_VERSION(__FILE__, "$Revision: 379392 $")
...
{
	/* T42.3: tmp is char *; parse propagates that. Was const here but the
	 * function writes through second_bracket via *second_bracket++='\0'
	 * (line 1051), so parse must remain writable to avoid the const-cast
	 * via strchr. tmp signature is char *, callers always pass writable. */
	char *parse = tmp;
	char *first_bracket;
	char *second_bracket;
	...
	*/
	while ( (first_bracket = strchr(parse, '<')) ) {
		...
		/* the bracket is within quotes, so ignore it */
		/* T42.4: find_closing_quote returns const char *; cast OK because
		 * parse here is the writable input buffer (tmp is char *). */
...
struct sip_via *parse_via(const char *header)
{
	...
	/* T42.3: v->sent_by is const char *; parm uses strchr return only for
	 * read (++parm rebinds; *(++parm)==':' reads; strtol(++parm,...) reads).
	 * Cast accepted because parm is reused as char * for strsep below. */
	if (((parm = (char *)strchr(v->sent_by, ']')) && *(++parm) == ':') || (parm = (char *)strchr(v->sent_by, ':'))) {
		char *endptr;
