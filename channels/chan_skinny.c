...
GABPBX_FILE_VERSION(__FILE__, "$Revision: 378303 $")
...
/*************************************
 * Skinny/GABpbx Protocol Settings *
 *************************************/
...
static void start_rtp(struct skinny_subchannel *sub)
{
	...
	sub->rtp = ast_rtp_instance_new("gabpbx", sched, &bindaddr_tmp, NULL);
	if (hasvideo)
		sub->vrtp = ast_rtp_instance_new("gabpbx", sched, &bindaddr_tmp, NULL);
...
static void *skinny_ss(void *data)
{
	...
	while (len < AST_MAX_EXTENSION-1) {
		...
		if (ast_exists_extension(c, c->context, d->exten, 1, l->cid_num)) {
			...
			&& ((d->exten[0] != '*') || (!ast_strlen_zero(d->exten) && strlen(d->exten) > 2))) {
...
static int skinny_indicate(struct ast_channel *ast, int ind, const void *data, size_t datalen)
{
	...
	case AST_CONTROL_RINGING:
		...
		return -1; /* Tell gabpbx to provide inband signalling */
	case AST_CONTROL_BUSY:
		if (ast->_state != AST_STATE_UP) {
			if (!d->earlyrtp) {
...
static int handle_offhook_message(struct skinny_req *req, struct skinnysession *s)
{
	...
	/* if any line on a device is offhook, than the device must be offhook, 
	   ...
	   but gabpbx does, so we may need to do more work.  Ugly, we should 
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Skinny Client Control Protocol (Skinny)",
		.load = load_module,
