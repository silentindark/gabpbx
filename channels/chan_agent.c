 ...
 * It is a dynamic module that is loaded by GABpbx. 
 * \par See also
...
GABPBX_FILE_VERSION(__FILE__, "$Revision: 380364 $")
...
/*! \brief Part of the GABpbx PBX interface */
static struct ast_channel *agent_request(const char *type, format_t format, const struct ast_channel* requestor, void *data, int *cause)
{
	struct agent_pvt *p;
...
static const struct ast_data_entry agents_data_providers[] = {
	AST_DATA_ENTRY("gabpbx/channel/agent/list", &agents_data_provider),
};

/*!
 * \brief Initialize the Agents module.
 * This function is being called by GABpbx when loading the module. 
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Agent Proxy Channel",
		.load = load_module,
