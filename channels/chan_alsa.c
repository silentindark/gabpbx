...
GABPBX_FILE_VERSION(__FILE__, "$Revision: 335064 $")
...
static int load_module(void)
{
	...
	if (soundcard_init() < 0) {
		...
		ast_verb(2, "Turn off ALSA support by adding 'noload=chan_alsa.so' in /etc/gabpbx/modules.conf\n");
		return AST_MODULE_LOAD_DECLINE;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "ALSA Console Channel Driver",
		.load = load_module,
