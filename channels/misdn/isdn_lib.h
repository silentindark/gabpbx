/*
 * Chan_Misdn -- Channel Driver for GABpbx
 *
 * Interface to mISDN
 *
...
struct misdn_party_id {
	 ...
	 * \note The name is currently obtained from GABpbx for
...
struct misdn_bchannel {
	...
	/*!
	 * \brief Dialed/Called information struct
	 * \note The number_type element is set to "dialplan" in /etc/gabpbx/misdn.conf for outgoing calls
	 */
	struct misdn_party_dialing dialed;

	/*! \brief Originating/Caller ID information struct
	 * \note The number_type element can be set to "localdialplan" in /etc/gabpbx/misdn.conf for outgoing calls
	 * \note The number element can be set to "callerid" in /etc/gabpbx/misdn.conf for outgoing calls
	 */
	struct misdn_party_id caller;

	/*! \brief  Incoming Caller ID string tag for special purpose
	 * \note The element can be set to "incoming_cid_tag" in /etc/gabpbx/misdn.conf for incoming calls
	 */
	char incoming_cid_tag[MISDN_MAX_NAME_LEN];

	/*! \brief Connected-Party/Connected-Line ID information struct
	 * \note The number_type element can be set to "cpndialplan" in /etc/gabpbx/misdn.conf for outgoing calls
	 */
	 ...
	 * \note This value is user configurable in /etc/gabpbx/misdn.conf
	 */
	...
	/*! \brief TRUE if AOCDtype and AOCD data are ready to export to GABpbx */
	 ...
	 * \note Collected from the incoming SETUP message and exported to GABpbx variable MISDN_URATE.
	 */
	int urate;
	 ...
	 * \note Contents exported and imported to GABpbx variable MISDN_KEYPAD
	 */
...
/* 	unsigned char cldsub[24]; */
	 ...
	 * \note Contents exported and imported to GABpbx variable MISDN_USERUSER
...
#ifdef MISDN_1_2
	/*! \brief The configuration string for the mISDN dsp pipeline in /etc/gabpbx/misdn.conf. */
	char pipeline[128];
#else
	 ...
	 * \note This value is user configurable in /etc/gabpbx/misdn.conf (echocancel)
	 */
	int ec_deftaps;
#endif
