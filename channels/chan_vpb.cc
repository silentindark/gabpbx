...
GABPBX_FILE_VERSION(__FILE__, "$Revision: 333507 $")
...
/* Use GABpbx Indication or VPB */
static int use_ast_ind=0;

/* Use GABpbx DTMF detection or VPB */
static int use_ast_dtmfdet=0;

static int relaxdtmf=0;

/* Use GABpbx DTMF play back or VPB */
...
static struct vpb_pvt {
	...
	struct ast_frame f, fr;           /*!< GABpbx frame interface */
...
*  Get gabpbx to do the bridging
*/
#define BAD_V4PCI_BRIDGE

/* This one enables a half duplex bridge which may be required to prevent high pitched
 * feedback when getting gabpbx to do the bridging and when using certain gain settings.
 */
/* #define HALF_DUPLEX_BRIDGE */

/* This is the Native bridge code, which GABpbx will try before using its own bridging code */
static enum ast_bridge_result ast_vpb_bridge(struct ast_channel *c0, struct ast_channel *c1, int flags, struct ast_frame **fo, struct ast_channel **rc, int timeoutms)
{
...
static int vpb_digit_begin(struct ast_channel *ast, char digit)
{
	/* XXX Modify this callback to let GABpbx control the length of DTMF */
	return 0;
}
static int vpb_digit_end(struct ast_channel *ast, char digit, unsigned int duration)
{
	...
	if (use_ast_dtmf) {
		ast_verb(4, "%s: vpb_digit: asked to play digit[%c] but we are using gabpbx dtmf play back?!\n", p->dev, digit);
		return 0;
	}
...
static void *do_chanreads(void *pvt)
{
	...
	while (!p->stopreads && p->owner) {
		...
		if (p->bridge) {
			...
			if (p->owner->_bridge) {
				ast_verb(5, "%s: chanreads: Got GABpbx bridge with [%s].\n", p->dev, p->owner->_bridge->name);
				bridgerec = 1;
			} else {
				bridgerec = 0;
			}
...
/*		if ((p->owner->_state != AST_STATE_UP) || !bridgerec) */
		...
		if (p->play_dtmf[0]) {
			...
			/* This DTMF is played by gabpbx and leads to an annoying trailing beep on CISCO phones */
			if (!ignore_dtmf) {
...
static enum ast_module_load_result load_module()
{
	...
	while (v){
		if (strcasecmp(v->name, "cards") == 0) {
			...
			use_ast_ind = 1;
			ast_log(LOG_NOTICE, "VPB driver using GABpbx Indication functions!\n");
			...
			use_ast_dtmf = 1;
			ast_log(LOG_NOTICE, "VPB driver using GABpbx DTMF play functions!\n");
		} else if (strcasecmp(v->name, "ast-dtmf-det") == 0) {
			use_ast_dtmfdet = 1;
			ast_log(LOG_NOTICE, "VPB driver using GABpbx DTMF detection functions!\n");
		} else if (strcasecmp(v->name, "relaxdtmf") == 0) {
			relaxdtmf = 1;
			ast_log(LOG_NOTICE, "VPB driver using Relaxed DTMF with GABpbx DTMF detections functions!\n");
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Voicetronix API driver");
