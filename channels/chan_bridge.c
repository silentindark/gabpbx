...
static int bridge_hangup(struct ast_channel *ast)
{
	...
	/* Deal with the GABpbx portion of it */
	ast->tech_pvt = NULL;
...
static struct ast_channel *bridge_request(const char *type, format_t format, const struct ast_channel *requestor, void *data, int *cause)
{
	...
	/* Try to grab two GABpbx channels to use as input and output channels */
...
/*! \brief Unload the bridge interaction channel from GABpbx */
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Bridge Interaction Channel",
	.load = load_module,
