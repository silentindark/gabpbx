...
GABPBX_FILE_VERSION(__FILE__, "$Revision: 360712 $")
...
static int jingle_create_candidates(struct jingle *client, struct jingle_pvt *p, char *sid, char *from)
{
	...
	for (tmp = p->ourcandidates; tmp; tmp = tmp->next) {
		...
		iks_insert_attrib(content, "name", "gabpbx-audio-content");
...
static struct jingle_pvt *jingle_alloc(struct jingle *client, const char *from, const char *sid)
{
	...
	ast_sockaddr_from_sin(&bindaddr_tmp, &bindaddr);
	tmp->rtp = ast_rtp_instance_new("gabpbx", sched, &bindaddr_tmp, NULL);
	tmp->parent = client;
	if (!tmp->rtp) {
...
static int jingle_transmit_invite(struct jingle_pvt *p)
{
	...
	ast_copy_string(p->audio_content_name, "gabpbx-audio-content", sizeof(p->audio_content_name));
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Jingle Channel Driver",
		.load = load_module,
