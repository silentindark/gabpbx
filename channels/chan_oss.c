...
openit:
	...
	if (setformat(o, O_RDWR) < 0) {	/* open device */
		...
		ast_verb(1, "Turn off OSS support by adding " "'noload=chan_oss.so' in /etc/gabpbx/modules.conf\n");
		goto error;
	}
	if (o->duplex != M_FULL)
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "OSS Console Channel Driver");
