...
struct mgcp_subchannel {
	/*! subchannel magic string.
	   Needed to prove that any subchannel pointer passed by gabpbx
...
static int mgcp_senddigit_begin(struct ast_channel *ast, char digit)
{
	...
	if (p->dtmfmode & MGCP_DTMF_INBAND || p->dtmfmode & MGCP_DTMF_HYBRID) {
		...
		res = -1; /* Let gabpbx play inband indications */
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Media Gateway Control Protocol (MGCP)",
		.load = load_module,
