...
GABPBX_FILE_VERSION(__FILE__, "$Revision: 365313 $")
...
static struct local_pvt *local_alloc(const char *data, format_t format)
{
	...
	tmp->reqformat = format; //germanico
//	tmp->reqformat = AST_FORMAT_G729A;
...
/*! \brief Unload the local proxy channel from GABpbx */
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Local Proxy Channel (Note: used internally by other modules)",
		.load = load_module,
