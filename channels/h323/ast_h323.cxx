 ...
 * ast_h323.cpp
 *
 * OpenH323 Channel Driver for GABPBX PBX.
 *			By  Jeremy McNamara
 ...
 *
 * This file is part of the chan_h323 driver for GABpbx
 *
...
const char *h323product  = "H.323 Channel Driver for GABpbx";
...
class PGABpbxLog : public PObject, public iostream {
	PCLASSINFO(PGABpbxLog, PObject);

	public:
	PGABpbxLog() : iostream(cout.rdbuf()) { init(&buffer); }
	~PGABpbxLog() { flush(); }

	private:
	PGABpbxLog(const PGABpbxLog &) : iostream(cout.rdbuf()) { }
	PGABpbxLog & operator=(const PGABpbxLog &) { return *this; }
...
static PGABpbxLog *logstream = NULL;

int PGABpbxLog::Buffer::overflow(int c)
{
	if (pptr() >= epptr()) {
		int ppos = pptr() - pbase();
...
int PGABpbxLog::Buffer::underflow()
{
	return EOF;
}

int PGABpbxLog::Buffer::sync()
{
							...
							H245_TerminalCapabilitySetReject & reject)
{
	...
	for (int i = 0; i < remoteCapabilities.GetSize(); ++i) {
		...
		case H323Capability::e_Audio:
			for (int x = 0; codecs[x].gabpbx_codec > 0; ++x) {
				if ((subType == codecs[x].h245_cap) && (!codecs[x].formatName || (!strcmp(codecs[x].formatName, (const char *)remoteCapabilities[i].GetFormatName())))) {
					int ast_codec = codecs[x].gabpbx_codec;
					int ms = 0;
					...
					if (h323debug) {
						cout << "Found peer capability " << remoteCapabilities[i] << ", GABpbx code is " << ast_codec << ", frame size (in ms) is " << ms << endl;
					}
...
//						sendUserInputMode = SendUserInputAsInlineRFC2833;
		...
		case H323Capability::e_Video:
			for (int x = 0; vcodecs[x].gabpbx_codec > 0; ++x) {
				if (subType == vcodecs[x].h245_cap) {
					...
					if ((subType != H245_VideoCapability::e_genericVideoCapability) ||
						...
						if (h323debug) {
							cout << "Found peer video capability " << remoteCapabilities[i] << ", GABpbx code is " << vcodecs[x].gabpbx_codec << endl;
						}
						peer_capabilities |= vcodecs[x].gabpbx_codec;
					}
					if (cap)
						delete(cap);
				}
...
void MyH323Connection::SetCapabilities(int caps, int dtmf_mode, void *_prefs, int pref_codec)
{
	...
	for (y = 0, x = -1; x < 32 + 32; ++x) {
		...
		case AST_FORMAT_SPEEX:
			/* Not real sure if GABpbx acutally supports all
	...
	if (dtmfMode) {
		...
		if (dtmfMode == H323_DTMF_INBAND) {
			...
			sendUserInputMode = SendUserInputAsTone;	/* RFC2833 transmission handled at GABpbx level */
		}
	}
...
PBoolean MyH323_ExternalRTPChannel::Start(void)
{
	...
	/* Notify GABpbx of remote RTP information */
...
int h323_set_gk(int gatekeeper_discover, char *gatekeeper, char *secret)
{
	...
	if (gatekeeper_discover) {
		...
		if (endPoint->SetGatekeeper(gkName, rasChannel)) {
			...
			/* XXX Maybe we should fire a new thread to attempt to re-register later and not kill gabpbx here? */
			return 1;
		}
	}
	return 0;
