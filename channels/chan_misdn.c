 ...
 * \file
 *
 * \brief the chan_misdn channel driver for GABpbx
 *
...
enum misdn_chan_state {
	...
	MISDN_EXTCANTMATCH,        /*!< when gabpbx couldn't match our ext */
...
/*! GABpbx created the channel (outgoing call) */
#define ORG_AST 1
...
struct chan_list {
	/*!
	 * \brief The "allowed_bearers" string read in from /etc/gabpbx/misdn.conf
	 */
	...
	/*!
	 * \brief TRUE if a channel can be hung up by calling gabpbx directly when done.
	 */
	int need_hangup;
	 ...
	 * \note The "noautorespond_on_setup" boolean read in from /etc/gabpbx/misdn.conf
	 */
	 ...
	 * \note The "incoming_early_audio" boolean read in from /etc/gabpbx/misdn.conf
	 */
	 ...
	 * \note The "faxdetect" option string read in from /etc/gabpbx/misdn.conf
	 * \note It is settable by the misdn_set_opt() application.
	 */
	int faxdetect;
	 ...
	 * \note The "faxdetect_timeout" value read in from /etc/gabpbx/misdn.conf
	 */
	...
	/*!
	 * \brief TRUE if we will use the GABpbx DSP to detect DTMF/Fax
	 * \note The "astdtmf" boolean read in from /etc/gabpbx/misdn.conf
	 */
	int ast_dsp;

	/*!
	 * \brief Jitterbuffer length
	 * \note The "jitterbuffer" value read in from /etc/gabpbx/misdn.conf
	 */
	int jb_len;

	/*!
	 * \brief Jitterbuffer upper threshold
	 * \note The "jitterbuffer_upper_threshold" value read in from /etc/gabpbx/misdn.conf
	 */
	...
	/*!
	 * \brief Associated GABpbx channel structure.
	 */
	 ...
	 * \note The "context" string read in from /etc/gabpbx/misdn.conf
	 */
	char context[AST_MAX_CONTEXT];

	/*!
	 * \brief The configured music-on-hold class to use for this call.
	 * \note The "musicclass" string read in from /etc/gabpbx/misdn.conf
	 */
	 ...
	 * \note The "far_alerting" boolean read in from /etc/gabpbx/misdn.conf
	 */
	int far_alerting;

	/*!
	 * \brief TRUE if NT should disconnect an overlap dialing call when a timeout occurs.
	 * \note The "nttimeout" boolean read in from /etc/gabpbx/misdn.conf
	 */
	int nttimeout;
	 ...
	 * \note The "overlapdial" value read in from /etc/gabpbx/misdn.conf
	 */
	int overlap_dial;
...
#define MISDN_GABPBX_TECH_PVT(ast) ast->tech_pvt
 ...
 * invocation request.
 *
 * \param chan GABpbx channel to operate upon.
...
/*!
 * \internal
 * \brief Convert the mISDN type of number code to GABpbx type of number code
 *
 * \param number_type mISDN type of number code.
 *
 * \return GABpbx type of number code
 */
...
/*!
 * \internal
 * \brief Convert the GABpbx type of number code to mISDN type of number code
 *
 * \param ast_number_type GABpbx type of number code.
 *
...
/*!
 * \internal
 * \brief Convert the mISDN numbering plan code to GABpbx numbering plan code
 *
 * \param number_plan mISDN numbering plan code.
 *
 * \return GABpbx numbering plan code
 */
...
/*!
 * \internal
 * \brief Convert the GABpbx numbering plan code to mISDN numbering plan code
 *
 * \param ast_number_plan GABpbx numbering plan code.
 *
...
/*!
 * \internal
 * \brief Convert the mISDN presentation code to GABpbx presentation code
 *
 * \param presentation mISDN number presentation restriction code.
 *
 * \return GABpbx presentation code
 */
...
/*!
 * \internal
 * \brief Convert the GABpbx presentation code to mISDN presentation code
 *
 * \param presentation GABpbx number presentation restriction code.
 *
...
/*!
 * \internal
 * \brief Convert the mISDN screening code to GABpbx screening code
 *
 * \param screening mISDN number screening code.
 *
 * \return GABpbx screening code
 */
...
/*!
 * \internal
 * \brief Convert the GABpbx screening code to mISDN screening code
 *
 * \param screening GABpbx number screening code.
 *
...
/*!
 * \internal
 * \brief Convert GABpbx redirecting reason to mISDN redirecting reason code.
 *
 * \param ast GABpbx redirecting reason code.
 *
...
/*!
 * \internal
 * \brief Convert the mISDN redirecting reason to GABpbx redirecting reason code
 *
 * \param q931 mISDN redirecting reason code.
 *
 * \return GABpbx redirecting reason code
 */
...
/* *INDENT-OFF* */
	...
	{ MISDN_EXTCANTMATCH,        "EXTCANTMATCH" },        /* when gabpbx couldn't match our ext */
...
static int read_config(struct chan_list *ch)
{
	...
	if (ch->originator == ORG_AST) {
		...
		/* ORIGINATOR GABpbx (outgoing call) */
 ...
 *
 * \param ast Current GABpbx channel
...
/*!
 * \internal
 * \brief Get the connected line information out of the GABpbx channel.
 *
 * \param ast Current GABpbx channel
...
/*!
 * \internal
 * \brief Copy the redirecting information out of the GABpbx channel
 *
 * \param bc Associated B channel
 * \param ast Current GABpbx channel
 *
...
/*!
 * \internal
 * \brief Copy the redirecting info into the GABpbx channel
 *
 * \param ast Current GABpbx channel
...
static int misdn_answer(struct ast_channel *ast)
{
	...
	if (!ast || !(p = MISDN_GABPBX_TECH_PVT(ast))) {
		return -1;
	}
...
{
	/* XXX Modify this callback to support GABpbx controlling the length of DTMF */
	return 0;
}
...
static int misdn_hangup(struct ast_channel *ast)
{
	...
	p = MISDN_GABPBX_TECH_PVT(ast);
	if (!p) {
		ast_mutex_unlock(&release_lock);
		return -1;
	}
	MISDN_GABPBX_TECH_PVT(ast) = NULL;
...
static struct ast_frame *misdn_read(struct ast_channel *ast)
{
	...
	if (!(tmp = MISDN_GABPBX_TECH_PVT(ast))) {
...
static int misdn_write(struct ast_channel *ast, struct ast_frame *frame)
{
	...
	if (!ast || !(ch = MISDN_GABPBX_TECH_PVT(ast))) {
		return -1;
	}
	...
	int timeoutms)
{
	...
	if (! p1_b || ! p2_b) {
		ast_log(LOG_NOTICE, "Falling back to GABpbx bridging\n");
...
static struct ast_channel *misdn_request(const char *type, format_t format, const struct ast_channel *requestor, void *data, int *cause)
{
	...
	if (!ast) {
		...
		ast_log(LOG_ERROR, "Could not create GABpbx channel for Dial(%s)\n", dial_str);
		return NULL;
	}
...
{
	struct chan_list *tmp = MISDN_GABPBX_TECH_PVT(chan);
...
static struct ast_channel *misdn_new(struct chan_list *chlist, int state,  char *exten, char *callerid, int format, const char *linkedid, int port, int c)
{
	...
	if (tmp) {
		...
		MISDN_GABPBX_TECH_PVT(tmp) = chlist;
		chlist->ast = tmp;
...
static void release_chan(struct chan_list *ch, struct misdn_bchannel *bc)
{
	...
	if (ast) {
		...
		ast_ch = MISDN_GABPBX_TECH_PVT(ast);
		MISDN_GABPBX_TECH_PVT(ast) = NULL;
		chan_misdn_log(1, bc->port,
...
static void do_immediate_setup(struct misdn_bchannel *bc, struct chan_list *ch, struct ast_channel *ast)
{
	...
	while (!ast_strlen_zero(predial)) {
		...
		if (ch->ast && MISDN_GABPBX_TECH_PVT(ch->ast)) {
...
cb_events(enum event_e event, struct misdn_bchannel *bc, void *user_data)
{
	...
	case EVENT_SETUP:
	{
		...
		/* Update gabpbx channel caller information */
		...
		if (!ast_strlen_zero(bc->redirecting.from.number)) {
			...
			/* Update gabpbx channel redirecting information */
		 ...
		 * will be used by GABpbx automatically. */
	...
	case EVENT_BCHAN_DATA:
		...
		if (!misdn_cap_is_speech(ch->bc->capability)) {
			...
			if (pfd.revents & POLLOUT) {
				chan_misdn_log(9, bc->port, "writing %d bytes to gabpbx\n", bc->bframe_len);
 ...
 *
 * \param chan GABpbx channel to operate upon. (Not used)
...
static int load_module(void)
{
	...
	*misdn_ports = 0;
	...
	ast_register_application("misdn_set_opt", misdn_set_opt_exec, "misdn_set_opt",
		...
		"    a - Have GABpbx detect DTMF tones on called channel\n"
		...
		"    i - Ignore detected DTMF tones, don't signal them to GABpbx,\n"
...
static int misdn_facility_exec(struct ast_channel *chan, const char *data)
{
	struct chan_list *ch = MISDN_GABPBX_TECH_PVT(chan);
	char *parse;
	unsigned max_len;
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Channel driver for mISDN Support (BRI/PRI)",
	.load = load_module,
