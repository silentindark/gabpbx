...
static struct gtalk_pvt *gtalk_alloc(struct gtalk *client, const char *us, const char *them, const char *sid)
{
	...
	ast_sockaddr_from_sin(&bindaddr_tmp, &bindaddr);
	if (!(tmp->rtp = ast_rtp_instance_new("gabpbx", sched, &bindaddr_tmp, NULL))) {
...
#endif
/*! \brief Unload the gtalk channel from GABpbx */
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Gtalk Channel Driver",
		.load = load_module,
