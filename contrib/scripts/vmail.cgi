#!/usr/bin/perl
#
# Web based Voicemail for GABpbx
#
...
$footer = "<hr><font size=-1><a href=\"http://www.gabpbx.org\">The GABpbx Open Source PBX</a> Copyright 2004-2008, <a href=\"http://www.digium.com\">Digium, Inc.</a></a>";
...
<TITLE>GABpbx Web-Voicemail</TITLE>
<BODY BGCOLOR="white">
$stdcontainerstart
<FORM METHOD="post">
...
sub check_login($$)
{
	...
	if (!$filename) {
		$filename = "/etc/gabpbx/voicemail.conf";
	}
#	print header;
#	print "Including <h2>$filename</h2> while in <h2>$category</h2>...\n";
	...
	while(<VMAIL>) {
		chomp;
		if (/include\s\"([^\"]+)\"$/) {
			($tmp, $category) = &check_login("/etc/gabpbx/$1", $category);
			if (length($tmp)) {
#				print "Got '$tmp'\n";
				return ($tmp, $category);
			}
...
sub check_login_users {
	...
	my ($found, $fullname) = (0, "");
	open VMAIL, "</etc/gabpbx/users.conf";
	while (<VMAIL>) {
		chomp;
		if (m/\[(.*)\]/) {
			if ($1 eq $mbox) {
...
sub validmailbox($$$$)
{
	...
	while (<VMAIL>) {
		chomp;
		if (/include\s\"([^\"]+)\"$/) {
			($tmp, $category) = &validmailbox($mbox, $context, "/etc/gabpbx/$1");
			if ($tmp) {
				return ($tmp, $category);
			}
		} elsif (/\[(.*)\]/) {
...
#	print "Including <h2>$filename</h2> while in <h2>$category</h2>...\n";
	...
	while(<VMAIL>) {
		...
		s/\;.*$//;
		if (/include\s\"([^\"]+)\"$/) {
			($tmp2, $category) = &mailbox_options($context, $current, "/etc/gabpbx/$1", $category);
#			print "Got '$tmp2'...\n";
			$tmp .= $tmp2;
		} elsif (/\[(.*)\]/) {
			$category = $1;
...
sub msgcount() 
{
	...
	my $path = "/var/spool/gabpbx/voicemail/$context/$mailbox/$folder";
	if (opendir(DIR, $path)) {
...
sub getfields()
{
	...
	my $fields;
	if (open(MSG, "</var/spool/gabpbx/voicemail/$context/$mailbox/$folder/msg${msg}.txt")) {
		while(<MSG>) {
			s/\#.*$//g;
			if (/^(\w+)\s*\=\s*(.*)$/) {
				$fields->{$1} = $2;
...
<TITLE>GABpbx Web-Voicemail: Preferences</TITLE>
<BODY BGCOLOR="white">
$stdcontainerstart
<FORM METHOD="post">
...
<TITLE>GABpbx Web-Voicemail: $folder Message $msgid</TITLE>
<BODY BGCOLOR="white">
$stdcontainerstart
<FORM METHOD="post">
<table width=100% align=center>
...
sub message_audio()
{
	...
	my $path = "/var/spool/gabpbx/voicemail/$context/$mailbox/$folder/msg${msgid}.$format";
...
<TITLE>GABpbx Web-Voicemail: $mbox\@$context $folder</TITLE>
<BODY BGCOLOR="white">
$stdcontainerstart
<FORM METHOD="post">
<table width=100% align=center>
...
sub message_rename()
{
	...
	my $path = "/var/spool/gabpbx/voicemail/$context/$mbox/$newfolder";
	$path =~ /^(.*)$/;
	$path = $1;
	mkdir $path, 0770;
	$path = "/var/spool/gabpbx/voicemail/$context/$mbox/$oldfolder";
	...
	foreach $oldfile (@files) {
		...
		if ($tmp =~ /^(msg${old}.\w+)$/) {
			...
			$newfile = "/var/spool/gabpbx/voicemail/$context/$mbox/$newfolder/$tmp";
...
sub message_copy()
{
	...
	my $path = "/var/spool/gabpbx/voicemail/$context/$newmbox";
	$path =~ /^(.*)$/;
	$path = $1;
	mkdir $path, 0770;
	$path = "/var/spool/gabpbx/voicemail/$context/$newmbox/INBOX";
	$path =~ /^(.*)$/;
	$path = $1;
	mkdir $path, 0770;
	$path = "/var/spool/gabpbx/voicemail/$context/$mbox/$oldfolder";
	...
	foreach $oldfile (@files) {
		...
		if ($tmp =~ /^(msg${old}.\w+)$/) {
			...
			$oldfile = $path . "/$tmp";
			$tmp =~ s/msg${old}/msg${new}/;
			$newfile = "/var/spool/gabpbx/voicemail/$context/$newmbox/INBOX/$tmp";
...
sub message_delete()
{
	...
	my $path = "/var/spool/gabpbx/voicemail/$context/$mbox/$folder";
	opendir(DIR, $path) || die("Unable to open directory\n");
...
sub message_forward()
{
	...
	my $path = "/var/spool/gabpbx/voicemail/$context/$newmbox/INBOX";
	if ($msgs[0]) {
		if (&lock_path($path) == 0) {
...
sub message_delete_or_move()
{
	...
	$path = "/var/spool/gabpbx/voicemail/$context/$mbox/$folder";
	if ($msgs[0]) {
		if (&lock_path($path) == 0) {
