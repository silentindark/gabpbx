...
GABPBX_FILE_VERSION(__FILE__, "$Revision: 350571 $")
...
static int process_my_load_module(struct ast_config *cfg)
{
	...
	if (!(tmp = ast_variable_retrieve(cfg, "global", "dbname"))) {
		ast_log(LOG_WARNING,"PostgreSQL database not specified.  Assuming gabpbx\n");
		tmp = "gabpbxceldb";
	}
	if (pgdbname)
		ast_free(pgdbname);
	...
	if (!(tmp = ast_variable_retrieve(cfg, "global", "user"))) {
		ast_log(LOG_WARNING,"PostgreSQL database user not specified.  Assuming gabpbx\n");
		tmp = "gabpbx";
	}
	if (pgdbuser)
		ast_free(pgdbuser);
