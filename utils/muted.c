 ...
 * Connects to the GABpbx Manager Interface, AMI, and listens for events
...
static char *config = "/etc/gabpbx/muted.conf";
...
/*! Connect to the gabpbx manager interface */
static int connect_gabpbx(void)
{
	int sock;
	struct hostent *hp;
	char *ports;
...
/*! Login to the gabpbx manager interface */
static int login_gabpbx(void)
{
	char *welcome;
	char *resp;
	if (!(welcome = get_line())) {
...
int main(int argc, char *argv[])
{
	...
	if (needfork) {
		...
		const char *found = NULL, *paths[] = {
			"/Library/LaunchAgents/org.gabpbx.muted.plist",
			"/Library/LaunchDaemons/org.gabpbx.muted.plist",
			"contrib/init.d/org.gabpbx.muted.plist",
			"<path-to-gabpbx-source>/contrib/init.d/org.gabpbx.muted.plist" };
		char userpath[256];
		struct stat unused;
